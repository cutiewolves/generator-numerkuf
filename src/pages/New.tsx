...
const New = () => {
  ...
  const handleGenerate = () => {
    ...
    if (finalNumber < parsedMin || finalNumber > parsedMax) {
        showError(`Numer ${finalNumber} jest poza zakresem od ${parsedMin} do ${parsedMax}.`);
        return;
    }

    if (finalNumber === parsedExcluded) {
        showError(`Numer ${finalNumber} jest wykluczony.`);
        return;
    }
    ...
    const randomJitterFactor = (Math.random() - 0.5) * 0.8;
